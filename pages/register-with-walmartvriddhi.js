...
//   };
  ...
  useEffect(() => {
    ...
    script.src =
      "https://www.google.com/recaptcha/api.js?render=6Lc01sspAAAAADM8fQcICBwBT2M62nMi-_UFynYm";
    script.async = true;
  ...
  const handleFromSectors = (e) => {
    const sector = sectorList.find((sector) => sector.name === e.target.value);
  ...
  const handleTextChange = (e) => {
    ...
    if (fname === "yourGst") {
      if (!gstRegex.test(value)) {
        console.log("invalid details");
        const fieldErrors = {};
        const { field, message } = gstErrors;
        fieldErrors[field] = message;
        setErrors(fieldErrors);
        //alert('valid email enter')
      } else {
        console.log("gst looks valid");
        setGst(value);
        setErrors();
      }
    }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const token = await new Promise((resolve, reject) => {
        window.grecaptcha
          .execute("6Lc01sspAAAAADM8fQcICBwBT2M62nMi-_UFynYm", {
            action: "submit",
          })
          .then(resolve)
          .catch(reject);
  ...
  function createPost() {
    ...
    axios
      ...
      .then((response) => {
        ...
        //   document.getElementById("yourGst").value = "";
  ...
  return (
    <>
      ...
      <Container className="reg-page p-0 " fluid>
        <p className="fs-4  text-center pt-5">
          Fill the form to be a part of the Walmart Vriddhi program and unlock
              ...
              <form style={{ margin: "20px" }}>
                <div className="mb-3">
                  <label className="form-label">
                    <span className="errors">*</span>Business Name:
                  </label>
                  <input
                    //required
                    type="text"
                    className={`form-control ${
                      errors && errors.yourBusiness ? "is-invalid" : ""
                    }`}
                    id="yourBusiness"
                    name="yourBusiness"
                    placeholder=""
                ...
                <label className="form-label">
                  <span className="errors">*</span>Enterprise:
                </label>

                <select
                  //required
                  className={`form-control ${
                    errors && errors.yourType ? "is-invalid" : ""
                  }`}
                ...
                {errors && errors.yourType && (
                  <div className="invalid-feedback">{errors.yourType}</div>
                )}

                <div className="mb-3 mt-3">
                  <label htmlfor="yourName" className="form-label">
                    <span className="errors">*</span>Your Name:
                  </label>
                  <input
                    //required
                    type="text"
                ...
                <label className="form-label">
                  <span className="errors">*</span>Business:
                </label>
                <select
                  //required
                  className={`form-control ${
                    errors && errors.yourSector ? "is-invalid" : ""
                  }`}
                  aria-label="Default select example"
                  name="yourSector"
                  value={yourSector}
                ...
                {errors && errors.yourSector && (
                  <div className="invalid-feedback">{errors.yourSector}</div>
                )}
                {/* gst field starts */}
                <div>
    ...
    {/* gst field ends */}
                ...
                <label className="form-label mt-2">
                  <span className="errors">*</span>State:
                </label>
                ...
                <select
                  ...
                  name="yourState"
                  className={`form-control ${
                    errors && errors.yourState ? "is-invalid" : ""
                  }`}
                ...
                {errors && errors.yourState && (
                  <div className="invalid-feedback">{errors.yourState}</div>
                )}

                <label className="form-label mt-2">
                  <span className="errors">*</span>City:
                </label>
                <select
                  className={`form-control ${
                    errors && errors.yourCity ? "is-invalid" : ""
                  }`}
                  id="yourCity"
                  name="yourCity"
                ...
                {errors && errors.yourCity && (
                  <div className="invalid-feedback">{errors.yourCity}</div>
                )}

                <div className="mb-3 mt-3">
                  <label htmlfor="yourPhone" className="form-label">
                    <span className="errors">*</span>Your Phone:
                  </label>
                  <input
                    //required
                    type="tel"
                ...
                <div className="mb-3 mt-3">
                  <label className="form-label">
                    <span className="errors">*</span>How did you hear about us?
                  </label>

                  <select
                    //required
                    className={`form-control ${
                      errors && errors.selectedSource ? "is-invalid" : ""
                    }`}
                    aria-label="Default select example"
                    id="selectedSource"
                    name="selectedSource"
                ...
                <button
                  type="submit"
                  className={`btn btn-primary register pb-4 ${
